...
public class ProjectionTests {
    ...
    *
    *   4. new 명령어를 활용한 프로젝션
    *       다양한 종류의 단순 값들을 DTO 로 바로 조회하는 방식으로 new 패키지명.DTO명을 쓰면 해당 dto로 바로 반환받을 수 있다.
    *       new 명령어를 사용한 클래스의 객체는 엔티티가 아니므로 영속성 컨텍스트에서 관리 되지 않는다.
    * */
    ...
    void 단일_엔티티_프로젝션_테스트(){
        ...
        // 불편해서 mybatis로 조회를 해 오면 영속성 컨텍스트에 들어가지 않음.
    ...
    /*
     *   조회하려는 컬럼 값이 1개인 경우 TypedQoery 로 반환 타입을 단일 값에 대해 지정할 수 있지만 다중 열 컬럼을 조회하는 경우 타입을 지정하지 못한다.
     *   그 때 TypeQuery 대신 Query 를 사용하여 Object[] 로 행의 정보를 반환 받아 사용한다.
     * */
