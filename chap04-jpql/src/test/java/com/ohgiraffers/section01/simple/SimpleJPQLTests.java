...
public class SimpleJPQLTests {
    ...
    * */
    ...
    /*
    *  jpql의 기본 문법
    *   SELECT, UPDATE, DELETE 등의 키워드 사용은 sql과 동일 하다.
    ...
    *   - getSingleResult() : 결과가 정확히 한 행일 경우 사용하며 없거나 많으면 에러가 발생한다.
    ...
    * */
    ...
    void distinct를_활용한_중복제거_여러행_조회_테스트(){
        ...
        //when
        String jpql = "SELECT DISTINCT m.categoryCode FROM menu_section01 as m";   //as 생략 가능
    ...
    void in_연산자를_활용한_조회_테스트(){
        ...
        //when
        String jpql = "SELECT m FROM menu_section01 m where m.categoryCode in(6,10)";   //as 생략 가능
        List<Menu> menuList = entityManager.createQuery(jpql, Menu.class).getResultList();  // 축약가능
    ...
    void like_연산자를_활용한_조회_테스트(){
        ...
        //when
        String jpql = "SELECT m FROM menu_section01 m where m.menuName like '%마늘%'";   //as 생략 가능
        List<Menu> menuList = entityManager.createQuery(jpql, Menu.class).getResultList();  // 축약가능
