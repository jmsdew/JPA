...
public class EmbeddedKeyTests {
    ...
    *   1. @EmbeddedId 어노테이션을 이용하는 방법이다.
    *       이 방법은 복합 키를 구성하는 필드들을 하나씩 클래스로 묶은 뒤, 해당 클래스를 @EmbeddedId 어노테이션을 사용하여 매핑하는 것이다.
    ...
    *
    *   복합 키의 매핑에서는 복합 키 클래스에 Equals 와 hashcode 메소드를 구현해야 한다는 점에 주의해야 한다.
    ...
    *   다만, EmbeddedId 가 조금 더 객체 지향다운 방법이고, @IdClass 는 관계형 데이터베이스에 가까운 방법이다.
    * */
