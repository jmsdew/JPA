...
public class ManyToOneAssociationTests {
    ...
    *   - 1:N(OneToMany) 연관관계
    *   - 1:1(OneToOne) 연관관계
    *   - N:N(ManyToMany) 연관관계  // 사용하지 않음. 매핑 테이블 이용 데이터 중복 저장 방지
    *
    *   방향에 따른 분류
    *   테이블의 연관 관계는 외래 키를 이용하여 양방향 연관 관계의 특징을 가진다.
    *   참조에 의한 객체의 연관 관계는 단방향이다.
    *   객체간의 연관관계를 양방향으로 만들고 싶은 경우 반대 쪽에서도 필드를 추가해서 참조를 보관하면 된다.
    ...
    *   예를 들어 하나의 카테고리가 여러 개의 메뉴를 가질 수 있는 상황에서 메뉴 엔티티가 카테고리를 참조하는 것이다.
    ...
    * */
    ...
    /*
    *   JPQL 은 java Persistence Query Language 의 약자로 , 객체 지향 쿼리 언어 중 하나이다.
    *   객체 지향 모델에 맞게 작성된 쿼리를 통해, 엔티티 객체를 대상으로 검색, 검색 결과를 토대로 객체를 조작할 수 있다.
    *   join 문법이 sql 과는 다소 차이가 있지만 직접 쿼리를 작성할 수 있는 문법을 제공한다.
    *   주의할 점은 FROM 절에 기술할 테이블 명에는 반드시 엔티티명이 작성되어야 한다.
    * */
    ...
    *   cascade=Cascade.PERSIST 를 설정하면 MenuAndCategory 엔티티를 영속화 할 때 Category 엔티티도 함께 영속화 하게 된다.
    * */
    ...
    void 다대일_연관관계_객체_삽입_테스트(){
        ...
        Category category = new Category();         // 카테고리가 부모요소 - 먼저 insert 됨
        category.setCategoryCode(33333);
        category.setCategoryName("신규카테고리");   //  CascadeType.PERSIST 설정이 되어있으면 update 발생 ex) 이름 변경
    ...
    void 영속성_삭제_테스트(){
        ...
        transaction.commit();
        MenuAndCategory deletMenu = entityManager.find(MenuAndCategory.class,100007);
        Category foundCategory = entityManager.find(Category.class, 33340);
    ...
    void Merge_update_테스트(){
        ...
        Category category = entityManager.find(Category.class,33334);
    ...
    void detach_테스트(){
        ...
        MenuAndCategory menuAndCategory = entityManager.find(MenuAndCategory.class, 100009);
        menuAndCategory.setMenuName("변경함");
        Category category = menuAndCategory.getCategory();
        category.setCategoryName("detach 카테고리 수정 함");
        menuAndCategory.setCategory(category);

        EntityTransaction transaction = entityManager.getTransaction();
        transaction.begin();

        entityManager.detach(menuAndCategory);   // 반영되지 않음. 준영속 상태 연관관계 맺은 것도   setMenuName 이것도 반영안됨
