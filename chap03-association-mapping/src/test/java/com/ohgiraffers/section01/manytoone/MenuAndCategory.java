...
public class MenuAndCategory {
    ...
    /*
    *  CascadeType.PERSIST
    *    PERSIST : 자식의 값이 저장될 때 연관관계를 가지고 있는 부모의 값도 함께 저장됨 / 엔티티를 영속화할 때, 연관된 하위 엔티티도 함께 유지한다.  -- 없는 값은 추가
    *    Merge : 엔티티 상태를 병합 할 때 연관된 하위 엔티티도 모두 병합한다.  -- 없는 값은 에러
...
//    @ManyToOne(cascade = CascadeType.REMOVE) // 테스트2 / 관계를 맺고 있는 요소까지 같이 지움
