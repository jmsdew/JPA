...
class ProblemsOfUsingDirectSQLTests {
    ...
    void closeConnection() throws SQLException {
        con.rollback();  // db에 쌓이지 않기 위해서 rollback 해줘야함
        con.close();
    }
    ...
    * */
    ...
    @Test
    void itemDirectSelectSql() throws SQLException {
        //given
    ...
    @DisplayName("직접 sqp을 작성하여 신규 메뉴를 추가할 떄 발생하는 문제 확인")
    @Test
    ...
    /*
    *   2. SQL 에 의존하여 개발
    *   요구사항의 변경에 따라 애플리케이션의 수정이 SQL 의 수정으로도 이어진다.
    *   이러한 수정 영향을 미치는 것은 오류를 발생시킬 가능성도 있지만 유지보수성에도 악영향을 미친다.
    *   또한 객체를 사용할 때 SQL에 의존하면 객체에 값이 무엇이 들어있는지 확인하기 위해 SQL 을 매번 살펴야한다.
    *
    * */
    ...
    *  * public class Menu {
    ...
    // 4. 동일성 보장 문제
    @DisplayName("조회한 두 개의 행을 담은 객체 동일성 비교 테스트")
    @Test
    void testEquals() throws SQLException {
        ...
        Assertions.assertEquals(menu , menu2);   // jpa는 true 가 나옴 영속성contaxt에 미리 데이터를 담아둠 항상 최신화
    }
}